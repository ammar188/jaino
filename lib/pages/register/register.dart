...
class RegisterController extends State<Register> {
  ...
  Future<void> sendVerificationEmail() async {
    ...
    if (email.isEmpty || !email.contains('@')) {
      showError(l10n.enterAnEmailAddress);
      return;
    }
    if (username.isEmpty) {
      showError(l10n.pleaseEnterYourUsername);
      return;
    } else if (!RegExp(r'^[a-z0-9._\-=/]+$').hasMatch(username)) {
      showError('Only lowercase letters, numbers and ._-=/ are allowed');
      return;
    }
    if (password.isEmpty) {
      showError(l10n.pleaseEnterYourPassword);
      return;
    ...
    if (confirmPassword.isEmpty) {
      ...
      showError(l10n.passwordsDoNotMatch);
      return;
    }
