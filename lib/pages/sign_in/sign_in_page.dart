...
class _SignInPageState extends State<SignInPage> {
  AsyncSnapshot<bool> _state = const AsyncSnapshot.nothing();
  bool _started = false;

  void _setState(AsyncSnapshot<bool> newState) {
    if (!mounted) return;
    setState(() {
      _state = newState;
    });
  ...
  void didChangeDependencies() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      final defaultServer = AppSettings.defaultHomeserver.value;

      if (defaultServer == null || defaultServer.isEmpty) {
        Logs().w('No default homeserver found');
        _setState(AsyncSnapshot.withError(
          ConnectionState.done,
          'No default homeserver',
        ));
        return;
      }

      final server = PublicHomeserverData(name: defaultServer);

      Logs().i('Auto connecting to: $defaultServer');

      await connectToHomeserverFlow(
        server,
        context,
        _setState,
        widget.signUp,
  ...
  Widget build(BuildContext context) {
    return LoginScaffold(
      body: Center(
        child: _state.connectionState == ConnectionState.waiting
